...
def DYNA(epsilon):
    ...
    # qvaluefunction[bins - 1, :, bins - 1, :, :] = 0
    ...
    while True and eps < 25000:
        ...
        while not terminal(s) and t < 200:
            ...
            if terminal(s_):
                ...
                qvaluefunction[ds[0], ds[1], ds[2], ds[3], a] += alpha*(R + gamma*np.max(qvaluefunction[ds_[0], ds_[1], ds_[2], ds_[3], :]) 
                    - qvaluefunction[ds[0], ds[1], ds[2], ds[3], a])
            model[(ds[0], ds[1], ds[2], ds[3])] = [a, R, (ds_[0], ds_[1], ds_[2], ds_[3])]
            # for i in range(n):
            #     indexs = np.random.choice(range(len(model)))
            #     rs =  list(model.keys())[indexs]
            #     ra = model[rs][0]
            #     rs_ = model[rs][2]
            #     rR = model[rs][1]
            #     if terminal(rs_):
            ...
            #     else:
            #         qvaluefunction[rs[0], rs[1], rs[2], rs[3], ra] += alpha*(rR + gamma*np.max(qvaluefunction[rs_[0], rs_[1], rs_[2], rs_[3], :]) 
            #             - qvaluefunction[rs[0], rs[1], rs[2], rs[3], ra])
            s = s_ 
            ds = dstate(s)
            t+=1
        # epsilon -= 0.00001
